...
public class SystemInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws IOException {
        ...
        // 不需要登录的接口,并且未登陆的用户  赋默认值
        if (allowAccess != null && user == null) {
        	LoginInfoUtil.setCountryCode(LangConstants.LANG_VN);
        	LoginInfoUtil.setLang(request.getHeader(HeaderConstants.Lang));
            return true;
        }
        ...
        SysCountry country = ApiBusinessRedisUtils.getCountryInfo(user.getCountryCode());
        if(country != null){
        ...
        UserStatusDTO dto = ApiBusinessRedisUtils.getUserStatus(user.getAccno(), user.getId());
        if (dto.getIsFrozen()) {
        ...
        LoginInfoUtil.setUserType(user.getUserType());
        LoginInfoUtil.setAccno(user.getAccno());
        //保存本次的token
    ...
    private void renewalToken(String token, AppLoginUser user) {
        ...
        try {
            ...
            if (renewalRecord == null) {
                //2、检查redis是否有续期记录
                if (ApiBusinessRedisUtils.isExistCurrentDayKey(RedisKeys.APP_ACTIVETOKEN, token)) {
                    //4、更新本地服务缓存
                    TokenRedisGuavaCacheUtils.putRenewal(token, SimpleConstant.ZERO);
                } else {
                    //3、若本地缓存和redis同时没有续期记录，则进行续期
                    ApiBusinessRedisUtils.setRenewalToken(token, user);
                    //TODO 临时加入，后期去掉
                    ApiBusinessRedisUtils.isExistCurrentDayKey(RedisKeys.APP_ACTIVETOKEN_ACCOUNT, user.getAccno());
                }
            }
        } catch (Exception e) {
