package com.onelive.pay.common.utils;
 ...
 */
@Slf4j
public class ApiBusinessRedisUtils extends RedisUtil {


    //设置用户token缓存,7天过期
    public static void setAccessToken(String account, String acctoken, String jsonStr) {
        setAppToken(acctoken, jsonStr, RedisTimeEnum.GLOBAL.getValue());
        setAppUserInfo(account, acctoken, RedisTimeEnum.GLOBAL.getValue());
    }
    ...
    public static void setAppUserInfo(String account, String acctoken, Long expire) {
        set(LoginInfoUtil.getMerchantCode() + RedisKeys.APP_LOGIN_USERINFO + account + ":" + acctoken, acctoken, expire);
    }
    ...
    public static AppLoginUser getLoginUserByToken(String token) {
        ...
        log.info("根据token获取LoginUser对象------redis---key：" + key);
        String result = get(key);
        log.info("根据token获取LoginUser对象---------result：" + result);
     ...
     * @return
     */
    public static Map<String, String> getLotteryLangNameMap(Integer lotteryId) {
        if (lotteryId == null) {
            return null;
        }
        String key = RedisKeys.LOTTERY_MAP_LANG_KEY + lotteryId;
        return get(key);
    }
     ...
     * @param lotteryId
     */
    public static void setLotteryLangNameMap(Map<String, String> langNameMap, Integer lotteryId) {
        String key = RedisKeys.LOTTERY_MAP_LANG_KEY + lotteryId;
        set(key, langNameMap);
    }
